        ...
        <div class="room-area pt-100 pb-70">
            <div class="container">
                ...
                <div class="row pt-45">
                    ...
                    @php 
                        $booking = App\Models\Booking::withCount('assign_rooms')->whereIn('id',$check_date_booking_ids)->where('rooms_id', $item->id)->get()->toArray();

                        $total_book_room = array_sum(array_column($booking,'assign_rooms'));
...
<a href="{{ route('search_room_details',$item->id.'?check_in='.old('check_in').'&check_out='.old('check_out').'&person='.old('person'))}}">
                                <img src="{{ asset('upload/roomimg/'.$item->image) }}" width="550" height="300" alt="Images">
                            </a>
                            <div class="content">
                                <h3>
<a href="{{ route('search_room_details',$item->id.'?check_in='.old('check_in').'&check_out='.old('check_out').'&person='.old('person'))}}">{{ $item['type']['name'] }}</a>
                    ...
                    <p class="text-center text-danger"> Sorry No Room Available </p>
